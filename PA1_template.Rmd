...
The daily mean is `r pander(day_mean)` steps and the daily median is `r pander(day_median)` steps. 
...
Maximum average across all days is in interval `r max_int`.  
...
num_NA
```
The number of NA's is `r num_NA`
...
```
The daily mean with filled in missing data is `r pander(day_mean_mod)` steps and the daily median with filled in missing data is `r pander(day_median_mod)` steps. The absolute difference is `r pander(abs(day_mean-day_mean_mod))` for the mean and `r pander(abs(day_median-day_median_mod))` for the median.   
